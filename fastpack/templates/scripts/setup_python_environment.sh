...
# If your pyproject.toml contains multiple dependencies sections and it is desired to install sections individually, 
# add an option below for each section and a variable for each section, ie new_section_flag.
usage() {
...
install_packages() {
    ...
    if [[ "${dev_flag}" != "true" && "${all_flag}" != "true" ]]; then
        dependencies=( "build" )
